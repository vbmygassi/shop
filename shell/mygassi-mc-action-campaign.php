...
logger("Starting: mygassi-mc-action-campaign.php");

$coll = Mage::getModel("salesrule/rule")->getCollection();
$campId = null;
foreach($coll as $camp){
	if(false !== strpos($camp->getName(), "Aktion")){
		$campId = $camp->getId();
	}
}

if(null === $campId){
	logger("No-Action-Campaign");
	exit(1);
}
...
$opts['subject'] = 'Eur 5.- Rabatt für Empfängerinnen der MyGassi (Aktion)!';
