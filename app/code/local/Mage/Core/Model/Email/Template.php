...
class Mage_Core_Model_Email_Template extends Mage_Core_Model_Template
{
	 ...
	 */
	if(0 === strpos($this->getProcessedTemplateSubject($variables), "MyGassi-Rechnung")){
		try{
			$sale = $variables["order"];
			$pdf = Mage::getModel("sales/order_pdf_invoice")->getPdf($sale->getInvoiceCollection());
			$attachment = $mail->createAttachment($pdf->render());
			$attachment->filename = $sale->getIncrementId() . ".pdf";
